...
class MailhogApi:
    ...
    def get_api_v2_messages(self, limit: int = 50) -> Response:
        ...
        :return:
        """
        with allure.step('Получение последних поступивших писем'):
    ...
    def get_token_from_last_email(self, reset_password: bool = False) -> str:
        ...
        :return:
        """
        with allure.step('Получение токена из последнего поступившего письма'):
            time.sleep(2)
    ...
    def get_token_by_login(self, login: str, attempt: int = 5, reset_password: bool = False):
        ...
        :return: token
        """
        with allure.step('Получение токена по логину пользователя'):
            if attempt == 0:
